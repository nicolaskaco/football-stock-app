 ...
 *
 * A player is suspended for jornada J if, in the immediately previous jornada (by fecha):
 *  - They received a red card, OR
 *  - Their cumulative yellow card count crossed a multiple of 5
 *
...
export function getSuspensionMap(jornadas, targetJornadaId, categoria) {
  ...
  const prevJornada = yearJornadas[targetIdx - 1];
  const prevPartido = (prevJornada.partidos || []).find((p) => p.categoria === categoria);
  if (!prevPartido) return suspensions;

  // Red cards in previous jornada → suspended
  (prevPartido.partido_eventos || [])
    .filter((e) => e.tipo === 'roja')
    .forEach((e) => {
      if (e.player_id) suspensions.set(e.player_id, { reason: 'Roja' });
    });
...
export function getCurrentSuspensionsByCategory(jornadas) {
  ...
  for (const cat of CATEGORIAS_PARTIDO) {
    if (nextIdx >= 1) {
      ...
      // All jornadas are in the past — check the last jornada for active suspensions
      // We simulate by treating it as if there's a "next" jornada after the last
      const lastJornada = yearJornadas[yearJornadas.length - 1];
      const lastPartido = (lastJornada.partidos || []).find((p) => p.categoria === cat);
      if (!lastPartido) { result.set(cat, new Map()); continue; }

      const suspensions = new Map();

      // Red cards in last jornada
      (lastPartido.partido_eventos || [])
        .filter((e) => e.tipo === 'roja' && e.player_id)
        .forEach((e) => suspensions.set(e.player_id, { reason: 'Roja' }));
