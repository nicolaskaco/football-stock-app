...
| `torneos` | Tournaments. Notable columns: `posicion_resultado` (integer 1–16, nullable), `resultado_playoff` (text `'campeon'`/`'subcampeon'`, nullable), `comentario_resultado` (text, nullable) |
...
| `user_permissions` | Role and permission flags per user email (RLS enabled — policies restrict reads/writes to `service_role` and admin-level callers; front-end access is controlled via Edge Function caller verification) |
...
| [PlayersTab.jsx](src/components/PlayersTab.jsx) | Player CRUD, document upload, history modal. Ficha Médica check (individual and bulk) maps `tipo_documento` → `idtipodocumento` (Cédula de Identidad=1, Pasaporte=2, Otro=3); only strips non-digits for Cédulas. Sticky Nombre column on horizontal scroll. Clicking a player name opens a read-only `PlayerForm` modal. Bulk actions (change category, toggle casita, hide, import from XLSX). Injury icon (Swiss cross) shown next to player name when injured. Injury CRUD button (admin only). "Comparar" button (indigo, Users icon) appears when 2-3 players are selected, opens `PlayerComparisonModal`. Admin-only status filter dropdown (defaults to "Solo activos"): filters by `status` field — `activo` (default), `cedido`, `transferido`, `egresado`, `dado de baja`. Color-coded `StatusBadge` shown next to player name for non-active statuses. Admin-only **inline editing** of individual player fields directly in the table row — clicking a cell value opens an in-place input; saving calls `updatePlayer` and refreshes global state. |
| [PlayersTabViatico.jsx](src/components/PlayersTabViatico.jsx) | Financial fields view with change-request flow. Complemento column shows the effective value (override if active) with a yellow "temp" badge and tooltip showing the expiry date. Contracted players with `incluir_viatico_export = true` show their `complemento` in the Complemento column (instead of `–`) and a "Contrato + $X" label (green badge + amount, `whitespace-nowrap`) in the Total column. Sticky Nombre column on horizontal scroll. Clicking a player name opens a read-only `PlayerFormViatico` modal. Only shows players with `status = 'activo'` (hides cedidos, transferidos, egresados, dados de baja). |
...
| [TorneosTab.jsx](src/components/TorneosTab.jsx) | Tournament list and management. Result badge column: gold **Campeón**, gray **Subcampeón**, or ordinal position (1er, 2do, etc.) |
...
| [TesoreroTab.jsx](src/components/TesoreroTab.jsx) | Tesorero view with three features: (1) **Congelar Viáticos** toggle (same `viaticos_congelados` app setting also shown in ConfiguracionTab) — when enabled the card turns amber and a configurable contact name input appears; (2) **Exportar Viáticos** — generates a multi-sheet Excel workbook (`Viaticos-Formativas-DD-MM-YYYY.xlsx`) with one sheet per formative category (Sub 19/17/16/15/14/13), sorted by name, excluding 3era; contracted players are excluded unless `incluir_viatico_export = true` — for those players, Total Viático = their `complemento` directly (not `calculateTotal`, which returns 0 for contracted players); each sheet includes a `TOTAL` / SUM formula row 3 rows below the last data row; (3) **Casos especiales** card — lists all contracted players in the 6 formative categories, each with an amber toggle that persists `incluir_viatico_export` on the player record; hidden when no contracted players exist in formative categories. |
...
- **PartidoForm**: Injured players shown with 🏥 prefix and injury type in the player select dropdowns (titulares and suplentes). Cross-category players shown with ⚠️ prefix and yellow background. Suspended players disabled with 🚫 prefix and red background.
...
- **EstadisticasTab "Árbitros" sub-tab**: Renders ArbitroPerformanceChart and ArbitroStatsTable. Table columns: Árbitro, PJ, G (with rival names), E (with rival names), P (with rival names), amarillas, rojas, Efect. %. Filterable by category. Chart tooltip shows rival names per result type. The **PJ** column is clickable — tapping a referee's match count opens a modal listing their partido details (fecha, rival, result).
...
A year filter dropdown (defaults to current year) scopes all jornadas to the selected year in both list and calendar views.
...
| `tk_search` | string | — |
| `tk_asignado` | assignee UUID \| `__sin_asignar__` | — |

Search matches on título, descripción, and asignado_nombre.

**Assignee filter dropdown** — positioned after the sprint selector in the toolbar. Shows only the Dirigentes and Funcionarios who actually have tasks assigned in the currently selected sprint (or across all sprints when "Todos los sprints" is active). Options are sorted alphabetically. "Sin Asignar" appears only when there are unassigned tasks in the current scope. The sentinel value `__sin_asignar__` matches tasks with no assignee. When switching sprints, the filter auto-resets to "Todos los asignados" if the previously selected assignee has no tasks in the new sprint.
...
### Suspension Utilities (`src/utils/suspensions.js`)
