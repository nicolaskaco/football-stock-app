...
- **SuspensionWidget** (OverviewTab): Shows players with 2+ yellows and currently suspended players; category filter pills; requires `can_view_tarjetas`. The 🟨 figure is the same running counter used for the suspension decision, so a player whose counter was wiped by a red card drops out of the list.
